...
// `transition-all` would animate layout properties too, so a button whose width
// changes would animate its own reflow. The press scale is what makes a control
// feel like it heard the click.
const buttonVariants = cva(
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-[color,background-color,border-color,box-shadow,transform] duration-(--duration-press) ease-out active:scale-[0.97] motion-reduce:active:scale-100 motion-reduce:transition-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none [&_svg]:pointer-events-none flex-shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive cursor-pointer select-none',
  {
    variants: {
      variant: {
        ...
        tertiary: 'bg-foreground/10 text-foreground shadow-none hover:bg-foreground/15',
        muted:
      ...
      size: {
        ...
        'icon-only': 'size-4 rounded-none p-0',
      },
    },
    defaultVariants: {
      variant: 'default',
